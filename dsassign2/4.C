...
class bst
{
	...
	int height(struct node *curr)
	{
		if(curr=NULL)
			return -1;
		return(max(height(curr->left))+max(height(curr->right))+1);
	}
	*/
	/*int count(struct node *curr)
	{
		...
		if(curr!=NULL)
		{
			...
			count(curr->right);
			c++;//delete curr;
			return c;//cout<<"inside destructor\n";
		}
	}
	int count1()
	{
	...
	while(1)
	{
		...
		{	
			...
			case 6:b.mirror(b.root);
				break;
			/*case 7:cout<<"height of tree "<<b.height<<endl;
				break;
			case 7:cout<<"count of tree "<<b.count1<<endl;
				break;*/
			case 7:flag=1;
				break;
